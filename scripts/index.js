    ...
    let selectedNodes = [];
        ...
        diagram.addDiagramListener("ObjectSingleClicked", (e) => {
            ...
            if (part instanceof go.Node) {
                console.log("Clic sur le nœud avec la clé :", part.key);
                console.log("selectedNodes avant traitement :", selectedNodes.map(node => node.key));
                if (selectedNodes.length < 2 && !selectedNodes.includes(part)) {
                    ...
                    if (selectedNodes.length >= 2) {
                        console.log("Deux nœuds déjà sélectionnés, action ignorée.");
                        return; // Empêcher la sélection d'un troisième nœud
                    }
                    selectedNodes = [part];
                    part.isSelected = true;
                }
                console.log("selectedNodes après traitement :", selectedNodes.map(node => node.key));
            }
        });
